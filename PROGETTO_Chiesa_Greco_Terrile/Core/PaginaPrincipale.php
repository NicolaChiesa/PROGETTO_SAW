<?php
  include('../Templates/Header.php');
?>

<div class="container py-5">
      <div class="mydiv1">
          <p class="scritta">Vuoi aggiungere un tocco<br> di stile alla tua casa ? <br>
                            In modo da stupire <br> i tuoi ospiti ?</p>
          <div class="polaroid">
            <img class="flex" src="../Immagini/ImmagineHome1.jpg"  alt="..." height="600"/>
          </div>
      </div>

      <div class = "mydiv1">
          <div class="polaroid">
            <img class="flex" src="../Immagini/vikings.png"  alt="..." height="380" />
          </div>    
          <p class="scritta">Riporta un po' <br> della magnifica storia <br> a casa tua. </p>
      </div>

      <div class = "mydiv1">
          <p class="scritta">Da noi puoi trovare <br> una grandissima raccolta<br>
          di articoli  mitologigi.</p>
          <div class="polaroid">
            <img class="flex" src="../Immagini/tuono.jpg"  alt="..." height="600" />
          </div>
      </div>  
</div>


<?php
  include('../Templates/Footer.php');
?>
